...
//! This version uses the typed pipeline. 
...
#[derive(Component, Reflect, Clone, Default, Deref, DerefMut, Serialize, Deserialize)]
#[reflect(Component)]
...
#[derive(Resource, Reflect, Clone, Default, Serialize, Deserialize)]
#[reflect(Resource)]
struct Scoreboard {
    score: usize,
}
...
fn handle_save_input(world: &mut World) {
    ...
    if keys.just_released(KeyCode::Space) {
        ...
        world.save_typed(BreakoutPipeline).expect("Failed to save");
        text = Some("Save");
    } else if keys.just_released(KeyCode::Back) {
        world.load_typed(BreakoutPipeline).expect("Failed to load");
        text = Some("Load");
